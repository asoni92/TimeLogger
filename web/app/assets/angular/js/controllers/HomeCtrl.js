var HomeCtrl = concurApp.controller('HomeCtrl', [
		...
		function($scope, $rootScope, APIFactory) {
			...
			$scope.intervalOptions = [{'key' : 'Day', 'value' : 'Daily'},{'key' : 'Month', 'value' : 'Monthly' } ,
			                          {'key' : 'Week', 'value' : 'Weekly'},{'key' : 'Quarter', 'value' : 'Quarterly' },
			                          {'key' : 'HalfYear', 'value' : 'HalfYearly'},{'key' : 'Year', 'value' : 'Yearly' }];
			
			
			
			$scope.changeUser = function(user) {
				$scope.filterOption.user = user;
			}
			
			$scope.changeInterval = function(option) {
				$scope.filterOption.interval = option;
			}
			
			$scope.getTableData = function() {
				APIFactory.getUserData().then( function(data) {
			...
			$scope.filterData = function() {
				var requestObj = {};
				if($scope.filterOption.user != null) {
					requestObj.userId = $scope.filterOption.user.id;
				}
				if($scope.filterOption.interval != null) {
					timePeriod : $scope.filterOption.interval.key;
				}
				APIFactory.getLoggerData(requestObj).then(
						function(data) {
							$scope.tableData = data != null ? data.data : [];
							var DTTime = 0;
							var IDTIme = 0;
							var MATime = 0;
							var STTime = 0;
			...
			$scope.getDescriptionType = function(desc) {
				var type = null;
				switch (desc) {
		          case 'OT':
		        	  type = "Other Task";
		        	  break;
		          case 'ST':
		        	  type = "Sustaining Task";
		        	  break;
		          case 'MA':
		        	  type = "Miscellaneous Activities";
		        	  break;
		          case 'ID':
			...
			$scope.getOtherTaskData = function(data) {
				var otData = [];
				if (data != null) {
			...
			$scope.createAreaChartData = function(data) {
				...
				if (data != null) {
					angular.forEach(data, function(v, k) {
						if (v.type == null || v.type == '' || v.type == 'OT') {
							var temp = [];
							temp.push(parseInt(v.dateCreated) * 1000);
							temp.push(parseInt(v.time));
							otData.push(temp)
						} else if (v.type == 'DT') {
							var temp = [];
							temp.push(parseInt(v.dateCreated) * 1000);
							...
							var temp4 = [];
							temp4.push(parseInt(v.dateCreated) * 1000);
							temp4.push(parseInt(v.time));
							$scope.stData.push(temp4)
						}
					})

				}
				console.log(JSON.stringify(otData))
				$scope.data2 = [{
						"key" : "North America" ,
					...
					{
						"key" : "Other Task" ,
						"values" : $scope.getOtherTaskData(data)
					}
				]
